...
void ops_par_loop_initialise_chunk_kernel_volume(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
    ...
    for ( int n_y=start_i; n_y<finish_i; n_y++ ){
      for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          initialise_chunk_kernel_volume(  (double * )p_a[0]+ i*1, (const double * )p_a[1]+ i*0, (double * )p_a[2]+ i*1,
           (const double * )p_a[3]+ i*1, (double * )p_a[4]+ i*1 );
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        initialise_chunk_kernel_volume(  (double * )p_a[0], (const double * )p_a[1], (double * )p_a[2],
           (const double * )p_a[3], (double * )p_a[4] );
  ...
  #ifdef OPS_DEBUG
  ops_dump3(arg0.dat,"initialise_chunk_kernel_volume");
  ops_dump3(arg2.dat,"initialise_chunk_kernel_volume");
  ops_dump3(arg4.dat,"initialise_chunk_kernel_volume");
  #endif
