...
void ops_par_loop_flux_calc_kernely(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
    ...
    for ( int n_y=start_i; n_y<finish_i; n_y++ ){
      for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          flux_calc_kernely(  (double * )p_a[0]+ i*1, (const double * )p_a[1]+ i*1, (const double * )p_a[2]+ i*1,
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        flux_calc_kernely(  (double * )p_a[0], (const double * )p_a[1], (const double * )p_a[2],
           (const double * )p_a[3] );
  ...
  #ifdef OPS_DEBUG
  ops_dump3(arg0.dat,"flux_calc_kernely");
  #endif
