...
void buildOpenCLKernels_flux_calc_kernely(int xdim0,
 int xdim1,
 int xdim2,
 int xdim3) {
  ...
  if(!isbuilt_flux_calc_kernely) {
    ...
    printf("Compiling flux_calc_kernely source -- start \n");
      ...
      if (pPath!=NULL)
        sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_flux_calc_kernely=%d -Dxdim1_flux_calc_kernely=%d -Dxdim2_flux_calc_kernely=%d -Dxdim3_flux_calc_kernely=%d", pPath, 32,xdim0,xdim1,xdim2,xdim3);
      else {
    ...
    OPS_opencl_core.kernel[33] = clCreateKernel(OPS_opencl_core.program, "ops_flux_calc_kernely", &ret);
    clSafeCall( ret );
...
void ops_par_loop_flux_calc_kernely(char const *name, ops_block Block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 4, sizeof(cl_mem), (void*) &OPS_opencl_core.constant[13]) );
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 5, sizeof(cl_int), (void*) &base0 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 6, sizeof(cl_int), (void*) &base1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 7, sizeof(cl_int), (void*) &base2 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 8, sizeof(cl_int), (void*) &base3 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 9, sizeof(cl_int), (void*) &x_size ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[33], 10, sizeof(cl_int), (void*) &y_size ));
