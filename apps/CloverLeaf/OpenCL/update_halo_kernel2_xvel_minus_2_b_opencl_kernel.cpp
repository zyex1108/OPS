...
static bool isbuilt_update_halo_kernel2_xvel_minus_2_b = false;

void buildOpenCLKernels_update_halo_kernel2_xvel_minus_2_b(int xdim0,
 int xdim1) {

  if(!isbuilt_update_halo_kernel2_xvel_minus_2_b) {
    buildOpenCLKernels();
    ...
    printf("Compiling update_halo_kernel2_xvel_minus_2_b source -- start \n");
      ...
      if (pPath!=NULL)
        sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_update_halo_kernel2_xvel_minus_2_b=%d -Dxdim1_update_halo_kernel2_xvel_minus_2_b=%d", pPath, 32,xdim0,xdim1);
      else {
      ...
      printf("compiling update_halo_kernel2_xvel_minus_2_b -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[56] = clCreateKernel(OPS_opencl_core.program, "ops_update_halo_kernel2_xvel_minus_2_b", &ret);
    clSafeCall( ret );

    isbuilt_update_halo_kernel2_xvel_minus_2_b = true;
  }
...
void ops_par_loop_update_halo_kernel2_xvel_minus_2_b(char const *name, ops_block Block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_update_halo_kernel2_xvel_minus_2_b(
  xdim0,
  xdim1);
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[56], 3, sizeof(cl_int), (void*) &base0 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[56], 4, sizeof(cl_int), (void*) &base1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[56], 5, sizeof(cl_int), (void*) &x_size ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[56], 6, sizeof(cl_int), (void*) &y_size ));
