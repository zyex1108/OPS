...
void ops_par_loop_advec_mom_kernel1_z_nonvector(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  #ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args,5,range,35)) return;
  #endif
    ...
    for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
      ...
      for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          advec_mom_kernel1_z_nonvector(  (double *)p_a[0]+ i*1*1, (double *)p_a[1]+ i*1*1, (double *)p_a[2]+ i*1*1,
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        advec_mom_kernel1_z_nonvector(  (double *)p_a[0], (double *)p_a[1], (double *)p_a[2],
