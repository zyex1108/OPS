...
void ops_par_loop_update_halo_kernel1_ba1(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6, ops_arg arg7) {
  ...
  #ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args,8,range,66)) return;
  #endif
    ...
    for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
      ...
      for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          update_halo_kernel1_ba1(  (double *)p_a[0]+ i*1*1, (double *)p_a[1]+ i*1*1, (double *)p_a[2]+ i*1*1,
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        update_halo_kernel1_ba1(  (double *)p_a[0], (double *)p_a[1], (double *)p_a[2],
