...
void ops_par_loop_update_halo_kernel4_plus_2_b(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
    ...
    for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
      ...
      for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          update_halo_kernel4_plus_2_b(  (double *)p_a[0]+ i*1*1, (double *)p_a[1]+ i*1*1, (int *)p_a[2] );
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        update_halo_kernel4_plus_2_b(  (double *)p_a[0], (double *)p_a[1], (int *)p_a[2] );
  ...
  if (OPS_diags > 1) {
    ...
    ops_timers_core(&c1,&t1);
    OPS_kernels[124].mpi_time += t1-t2;
    OPS_kernels[124].transfer += ops_compute_transfer(dim, range, &arg0);
    OPS_kernels[124].transfer += ops_compute_transfer(dim, range, &arg1);
  }
}
